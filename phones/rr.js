...
function H(select, p1,p2,p3,p4,p5,p6,p7){
for (let i = 0; i < phones.length; i++){
    if(select.value == phones[i].name){
        ...
        document.querySelector(p6).innerHTML = phones[i].display;
        document.querySelector(p7).innerHTML = phones[i].memory;
    }
}
}

selectLeft.onchange = function(){
    H(selectLeft, '.name-left', '.ekran-left', '.security-left', '.batery-left', '.procesor-left', '.display-left', '.memory-left')
}

selectRight.onchange = function(){
    H(selectRight, '.name-right', '.ekran-right', '.security-right', '.batery-right', '.procesor-right', '.display-right', '.memory-right')
}
